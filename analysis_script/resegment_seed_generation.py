...
output_path = "/Users/binxu/Connectomics_Code/results/LGN/"
if len(sys.argv)>2:
...
def worker_func(id_pair):
    ...
    mask = (composite_map_sh == cur_idx1 + cur_idx2 * BASE) | (composite_map_sh == cur_idx2 + cur_idx1 * BASE)
...
result = pool.map(worker_func, pair_list[:2])
#%%
seed_dict = {}
for result_vec, id_pair in zip(result, pair_list):
    cur_idx1, cur_idx2 = id_pair[0], id_pair[1]
    seed_dict[(cur_idx1, cur_idx2)] = seed_dict.get((cur_idx1, cur_idx2), []).extend(result_vec)  # note extend here
