...
def global_x_sel(i):
    if i == 0: return slice(0, x_step + x_margin)
    elif i == x_num-1: return slice((x_num - 1) * x_step + x_margin, x_num * x_step + 2 * x_margin)
    else: return slice((i - 1) * x_step + x_margin, i * x_step - x_margin)

def global_y_sel(i):
    if i == 0: return slice(0, y_step + y_margin)
    elif i == y_num-1: return slice((y_num - 1) * y_step + y_margin, y_num * y_step + 2 * y_margin)
    else: return slice((i - 1) * y_step + y_margin, i * y_step - y_margin)

def local_x_sel(i):
    if i==0:  return slice(0, -x_margin)
    elif i==x_num-1:  return slice(x_margin, None)
    else:  return slice(x_margin, -x_margin)
