...
**Size of Field** Usually the following sizes are the same, `train_image_size`,`train_canvas_size`,`train_eval_size`,`train_labels_size`  = `input_seed_size+deltas*2*fov_move()` = `pred_mask_size+deltas*2*fov_move()` = `fov_size + deltas*2*fov_move()` defaultly `fov_move()=fov_move=1` if use `max_pred_moves` then `+1`. 

**Permutation problem**: since the system defaultly perform augmentation by permute axis and reflect axis. 
So when the `fov_size` are not equal in x, y direction (like `55, 37, 17`),  then the extra argument `--permutable_axes 0` is crucial because by default the training program perform **augmentation** by permutating axis, so is the 2 axis are not equal in some sense (e.g. `fov_size` not equal ) then better stop permutation! 
...
### Overall Usage 
Inference part is majorly request based, which is input request string and make the system Run it. (Can be done interactively)
```python
...
Core Instance `inference.Runner()`. It has all the relavent method to manage and segment a `canvas` object! 

`runner.start()`
Note: `import_symbol`  is an amazing utility ! which can dynamically import sth from the string request. 
in `make_canvas` (inference.py 1054)there is a part to match histogram of target image stack and reference histogram from input. 
...
## Agglomeration / Resegmentation
Main function defined in `inference.resegmentation`
```python
...
runner.start(reSeg_req.inference) 
text_format.Parse(config, reSeg_req)
resegmentation.process(reSeg_req, runner)
```
From the paper we know that the merging is done by such criterion 

1. Choose 2 labels that are closely neighbored 

This is done in the medium of storage system 
