...
def compute_partitions_parallel(seg_array,
                                thresholds,
                                lom_radius,
                                id_whitelist=None,
    ...
    def worker_func(l):
        ...
        output_sh[object_mask & (active_fraction >= thresholds[-1]) &
                  (output_sh == 0)] = len(thresholds) + 1
        # mark the output w.r.t the active_fraction (to find the center / border of an object)
        logging.info('Done processing %d. Time %.3f s' % (l, time() - t1))
