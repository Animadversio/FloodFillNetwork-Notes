...
def threshold_segmentation(segmentation_dir, corner, labels, threshold):
  ...
  with gfile.Open(prob_path, 'rb') as f:  # NOTE: Add 'rb' to be runable from python3
    data = np.load(f)
...
def load_origins(segmentation_dir, corner):
  ...
  with gfile.Open(target_path, 'rb') as f:  # NOTE: Add 'rb' to be runable from python3
    data = np.load(f)
...
def load_segmentation(segmentation_dir, corner, allow_cpoint=False,
  ...
  with gfile.Open(target_path, 'rb') as f: #FIXME: 'r' will cause error, but 'rb' is fine
    data = np.load(f)
