...
class Canvas(object):
  ...
  def restore_checkpoint(self, path):
    ...
    with gfile.Open(path, 'rb') as f: # fixed on Feb.10th
      data = np.load(f)
...
class Runner(object):
  ...
  def make_canvas(self, corner, subvol_size, **canvas_kwargs):
    ...
    restrictor = self.make_restrictor(dst_corner, dst_size, image, alignment) # have seed mask and normal mask within
